...
module.exports = async (date = moment(), generated_by = 0, options = { ignore_bills: false, reading_from: null }) => {
    ...
    let previous = (await db.raw(`
        SELECT consumer_id, unit_code, period_to as last_reading_date, current_reading as last_reading, previous_reading_id
            ...
            try{
                forEach(consumers, async function(consumer){
                    ...
                    try{
                        ...
                        let previous_reading_id = 0;
                        if(options?.reading_from){
                            period_from = options.reading_from.format('YYYY-MM-DD');
                            readings.whereBetween("date",[period_from,to]);
                            let reading = await trx("water_readings").orderBy('date').where('consumer_id',consumer.id).where(z=>{
                                z.where("date",">=",period_from);
                                z.where("date","<",to);
                            }).select("value","id").first();
                            previous_reading = reading?.value || 0;
                            previous_reading_id = reading?.id || 0;
                            ...
                            previous_reading_id = previous_readings[consumer.id].previous_reading_id;
                        }
                            ...
                            forEach(readings, async function(r){
                                ...
                                try{
                                    ...
                                    if(!period_from){
                                        ...
                                        previous_reading_id = r.id;
                                        period_from = period_to.format("YYYY-MM-DD");
                                        return done();
                                    }
                                    const check = await trx("water_statements").where("previous_reading_id",previous_reading_id).first().whereNull("voided_on");
                                    if(check){
                                        console.log(`Reading id ${previous_reading_id} is already on an active statement`);
                                        return done();
                                    }
