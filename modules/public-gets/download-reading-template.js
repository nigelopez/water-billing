...
module.exports = async (data,req,res) => {
    return await db.transaction(async trx=>{
        ...
        // worksheet.cell(row,4).string("LAST READING DATE").style(headerStyle);
        // worksheet.column(4).setWidth(25);
        worksheet.cell(row,2).string("NEW READING VALUE").style(headerStyle);
        worksheet.column(2).setWidth(25);
        // worksheet.cell(row,7).string("WARNING").style(headerStyle);
        ...
        //                 worksheet.cell(row,1).string(c.unit_code);
        //                 worksheet.cell(row,2).string(c.name);
        //                 worksheet.cell(row,3).string(c.meter_number);
        //                 worksheet.cell(row,4).date(moment(reading.date).format("YYYY-MM-DD")).style(dateStyle);
        //                 worksheet.cell(row,5).number(reading.value).style(oldStyle);
        //                 worksheet.cell(row,6).style(newStyle);
        //                 worksheet.cell(row,7).formula(`=IF(ISBLANK(${excel.getExcelCellRef(row,7)}),"",IF(${excel.getExcelCellRef(row,7)}-${excel.getExcelCellRef(row,6)} < 0,"New reading value is below the last reading value!",""))`).style(warningStyle);
        //                 row++;
        //             }
        //         }catch(e){
        //             reject(e);
